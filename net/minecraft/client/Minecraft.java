...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	private void displayDebugInfo(long elapsedTicksTime) {
		if (this.mcProfiler.profilingEnabled) {
			...
			worldrenderer
					.pos((double) ((float) j - (float) i * 1.1F), (double) ((float) k - (float) i * 0.6F - 16.0F), 0.0D)
					.color(200, 0, 0, 0).endVertex();
			worldrenderer.pos((double) ((float) j - (float) i * 1.1F), (double) (k + i * 2), 0.0D).color(200, 0, 0, 0)
					.endVertex();
			worldrenderer.pos((double) ((float) j + (float) i * 1.1F), (double) (k + i * 2), 0.0D).color(200, 0, 0, 0)
					.endVertex();
			worldrenderer
			...
			for (int l = 0; l < list.size(); ++l) {
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					...
					worldrenderer.pos((double) ((float) j + f1), (double) ((float) k - f2), 0.0D).color(k1, l1, i2, 255)
							.endVertex();
				}
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					...
					worldrenderer.pos((double) ((float) j + f4), (double) ((float) k - f5), 0.0D)
							.color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
					worldrenderer.pos((double) ((float) j + f4), (double) ((float) k - f5 + 10.0F), 0.0D)
							.color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
				}
			...
			this.fontRendererObj.drawStringWithShadow(s = decimalformat.format(profiler$result.field_76330_b) + "%",
					(float) (j + i - this.fontRendererObj.getStringWidth(s)), (float) (k - i / 2 - 16), l2);
			...
			for (int k2 = 0; k2 < list.size(); ++k2) {
				...
				this.fontRendererObj.drawStringWithShadow(s1, (float) (j - i), (float) (k + i / 2 + k2 * 8 + 20),
				...
				this.fontRendererObj.drawStringWithShadow(
						...
						(float) (k + i / 2 + k2 * 8 + 20), profiler$result2.func_76329_a());
				this.fontRendererObj.drawStringWithShadow(
						s1 = decimalformat.format(profiler$result2.field_76330_b) + "%",
						(float) (j + i - this.fontRendererObj.getStringWidth(s1)), (float) (k + i / 2 + k2 * 8 + 20),
						profiler$result2.func_76329_a());
			}
		}
	}
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
			...
			while (Keyboard.next()) {
				...
				if (Keyboard.getEventKeyState()) {
					...
					EventKeyboard keyBoardEvent = new EventKeyboard(k);
					keyBoardEvent.call();
	...
	private void middleClickMouse() {
		if (this.objectMouseOver != null) {
			...
			EventMiddleClick e = new EventMiddleClick();
			e.call();
